...
Si le projet a une base de code avec relations explicites entre fichiers (imports en Python/JS, #include en C/C++, etc.), générer **automatiquement** un graphe de dépendances visualisable dans la KB.

### Pattern de référence : Python + JS/JSX (full-stack)

Créer `docs/kb/tools/depgraph.py` avec un **double scanner bi-langage** :

**Scanner backend (Python)** :
- Glob : `backend/app/**/*.py` (exclure `__pycache__`, `alembic/`, tests).
- Regex : `from app\.(\S+) import` et `import app\.(\S+)`.
- Groupement par couche : `routers`, `services`, `models`, `schemas`, `domain`, `application`, `core`.
- Représentation nœud : cercle (`ellipse` Cytoscape).

**Scanner frontend (JS/JSX)** :
- Glob : `frontend/src/**/*.{js,jsx}` (exclure `__tests__/`, `.test.`, `.spec.`).
- Regex : imports relatifs (`from ['"](\./|\.\./)[^'"]+['"]`).
- Résolution des chemins relatifs depuis le fichier source.
- Groupement par couche : `pages`, `components`, `api`, `hooks`, `contexts`, `utils`, `lib`.
- Représentation nœud : rectangle arrondi avec bordure pointillée (`round-rectangle` Cytoscape).

**3 artefacts générés** dans `docs/kb/docs/reference/` :
- `depgraph.json` : `{ nodes: [{id, label, layer, lang}], edges: [{source, target}], stats: {} }`.
- `depgraph.md` : page **Mermaid** avec vue agrégée par couche (illisible si >50 nœuds → utiliser la vue par dossier, pas par fichier).
- `depgraph-interactive.md` : page HTML intégrant **Cytoscape.js** (CDN unpkg) avec layout `fcose` (ou `dagre` en fallback), filtres : recherche, couche, langage (Python/JS), masquage des nœuds isolés.

**Cytoscape.js** est préféré à vis-network : meilleure performance sur 200+ nœuds, layouts hiérarchiques natifs (dagre), et API de style plus expressive.

**Ajouts dans kb.sh** :
```bash
cmd_graph() {
  require_python
  python3 "${SCRIPT_DIR}/tools/depgraph.py"
}
# case: graph) cmd_graph ;;
```

Ajouter `graph` dans l'en-tête de `kb.sh` et dans le `case`.

Référencer les deux pages dans le `nav` sous `Référence` :
```yaml
- Graphe (Mermaid): reference/depgraph.md
- Graphe (interactif): reference/depgraph-interactive.md
```

Documenter la régénération dans `depgraph.md` via une admonition `!!! tip`.

**Alternative outillée** : `pydeps` (Python), `madge` (JS/TS), `cflow`/`callgraph` (C), `jdeps` (Java) → transformer la sortie en JSON pour Cytoscape.
